...
def run_free_adv_training(m):
    ...
    model = defenses.free_adv_train(model, data_tr, criterion, optimizer, \
                                    scheduler, consts.PGD_Linf_EPS, device, m=m)
    train_time = time.time()-t0

    # move model to cpu and store it
    model.to('cpu')
    torch.save(model.state_dict(), \
               f'trained-models/simple-cnn-free-adv-trained_{m}')

    # done
    return train_time


def run_evaluation(m):

    # load standard and adversarially trained mdoels
    trained_models = {}
    mpaths = {'adv_trained': f'trained-models/simple-cnn-free-adv-trained_{m}'}
    for mtype in mpaths:
        model = models.SimpleCNN()
